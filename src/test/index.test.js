...
describe("backwardsify()", () => {
  it("backwardsify() method exists", () => {
    expect(backwardsify).to.exist;
  });
...
describe("doubleNumbers()", () => {
  it("doubleNumbers() method exists", () => {
    expect(doubleNumbers).to.exist;
  });
...
describe("doubleCharacters()", () => {
  it("doubleCharacters() method exists", () => {
    expect(doubleCharacters).to.exist;
  });
...
describe("interleave()", () => {
  it("interleave() method exists", () => {
    expect(interleave).to.exist;
  });
  ...
  it("on different input array sizes, return null", () => {
    expect(
      interleave([1], [2, 3]),
      "different size input arrays must return null"
    ).to.eq(null);
  });

  it("correctly merged values from 2 arrays into single result", () => {
    expect(interleave(["a", "b", "c"], ["x", "y", "z"])).to.deep.eq([
      "a",
      "x",
      "b",
      "y",
    ...
    expect(interleave([10, 20, 30, 40], [5, 6, 7, 8])).to.deep.eq([
      10,
      5,
      20,
      6,
...
describe("makeRange()", () => {
  it("makeRange() method exists", () => {
    expect(makeRange).to.exist;
  });
...
describe("countByFirstLetter()", () => {
  ...
  it("countByFirstLetter() method exists", () => {
    expect(countByFirstLetter).to.exist;
  });
...
// Here's the corrected groupByFirstLetter test code:
